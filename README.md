...
「ライブラリ」タブ → 右上の「…」（その他の操作）→「通信診断とログ」から通信診断を実行し、「ログを書き出す」でテキストファイルを保存・共有できます。LiveContainerで番組取得や再生に失敗する場合は、問題を再現した直後に書き出してください。ログにはOS・アプリバージョン、TVer/Streaksへの疎通結果、番組表取得・再生の失敗段階が含まれます。トークン、Cookie、リクエストヘッダー、URLクエリ、実際の配信URLは記録されません。

## 現在の機能

- SwiftUI / `NavigationStack` を使ったアプリシェル
- 「見逃し」「番組表」「ライブ」「ライブラリ」の4タブ
- 見逃し一覧の常設検索・絞り込み・並び替え、配信期限とダウンロード状態の表示
- 番組表の日付移動・現在位置への移動、一覧／グリッド切り替え（アクセシビリティ文字サイズでは一覧）
- 保存済み動画・進行中の取得・マイリスト・視聴履歴・シリーズ自動取得を切り替えられるライブラリ
- ダウンロードの再開と最初からのやり直しを区別し、途中データを削除する前に確認
- 番組・配信日のデータモデル
- TVer のエピソード URL 生成
...
UIの変更内容と再現可能な検証手順は [UI・操作性の改修記録](docs/ui-usability-review.md) を参照してください。
